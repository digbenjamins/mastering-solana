...
const delegate = await loadSignerFromFile("ben.json"); // BENjdUV4wjhFPbEQfB3pnuC9g7PxLHH3kGE33oovG6WP
const mint = address(config.MINT);

/**
 *  Use findAssociatedTokenPda to derive the ATA address
 */
const [ownerAssociatedTokenAddress] = await findAssociatedTokenPda({
  mint: mint,
...
const approveInstruction = getApproveCheckedInstruction({
  source: ownerAssociatedTokenAddress,
  mint: mint,
  delegate: delegate.address,
  owner: wallet,
  amount: tokensToRaw(300, 9), // 9 decimals
  decimals: 9,
});
...
const transactionMessage = pipe(
  ...
  (tx) => appendTransactionMessageInstructions([approveInstruction], tx) // Append instructions
);
